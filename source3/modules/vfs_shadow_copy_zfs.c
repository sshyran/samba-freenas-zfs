				     ...
				     void *value, size_t size)
{
	...
	if (shadow_copy_zfs_match_name(fname, &gmt_start)) {
		...
		ret = SMB_VFS_NEXT_GETXATTR(handle, conv, aname, value,
					    size);
		saved_errno = errno;
		TALLOC_FREE(conv);
		errno = saved_errno;
				 ...
				 size_t size, int flags)
{
	...
	if (shadow_copy_zfs_match_name(fname, &gmt_start)) {
		...
		ret = SMB_VFS_NEXT_SETXATTR(handle, conv, aname, value, size,
					    flags);
		saved_errno = errno;
		TALLOC_FREE(conv);
		errno = saved_errno;
...
static int shadow_copy_zfs_get_quota(vfs_handle_struct *handle, const char *path,
